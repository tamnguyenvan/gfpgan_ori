...
import numpy as np
import torch
from gfpgan.archs.stylegan2_arch import (
    ConvLayer,
    EqualConv2d,
    EqualLinear,
    ResBlock,
...
class StyleGAN2GeneratorSFT(StyleGAN2GeneratorClean):
    ...
    def forward(
        ...
        noise = [
            torch.from_numpy(np.random.randn(1, 1, 4, 4)).float(),
            torch.from_numpy(np.random.randn(1, 1, 8, 8)).float(),
            torch.from_numpy(np.random.randn(1, 1, 8, 8)).float(),
            torch.from_numpy(np.random.randn(1, 1, 16, 16)).float(),
            torch.from_numpy(np.random.randn(1, 1, 16, 16)).float(),
            torch.from_numpy(np.random.randn(1, 1, 32, 32)).float(),
            torch.from_numpy(np.random.randn(1, 1, 32, 32)).float(),
            torch.from_numpy(np.random.randn(1, 1, 64, 64)).float(),
            torch.from_numpy(np.random.randn(1, 1, 64, 64)).float(),
            torch.from_numpy(np.random.randn(1, 1, 128, 128)).float(),
            torch.from_numpy(np.random.randn(1, 1, 128, 128)).float(),
        ]
        # style truncation
        if truncation < 1:
...
class GFPGANv1(nn.Module):
    ...
    def __init__(
        ...
        channels = {
            ...
            '32': int(256 * channel_multiplier * unet_narrow),
            '64': int(128 * channel_multiplier * unet_narrow),
            '128': int(64 * channel_multiplier * unet_narrow),
            '256': int(32 * channel_multiplier * unet_narrow)
        }
    ...
    def forward(self, x, return_latents=False, return_rgb=False, randomize_noise=True):
