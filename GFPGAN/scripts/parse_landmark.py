...
# Configurations
save_img = True
scale = 0.25  # 0.25 for official FFHQ (512x512), 1 for others
enlarge_ratio = 1.4  # only for eyes
json_path = 'ffhq-dataset-v2.json'
face_path = '/content/BasicSR/ffhq_256'
save_path = './FFHQ_eye_mouth_landmarks_256.pth'
...
paths = [os.path.join(face_path, p) for p in os.listdir(face_path)]

save_dict = {}

for item_idx, item in enumerate(json_data.values()):
    print(f'\r{item_idx} / {len(json_data)}, {item["image"]["file_path"]} ', end='', flush=True)
    filename = Path(item['image']['file_path']).name.split('.')[0] + '.jpg'
    ...
    if save_img:
        ...
        with open(os.path.join(face_path, filename), 'rb') as f:
            img_bytes = f.read()
